...
/*   run; */
  ...
  data test_missing(keep = loan_id &train_var next_stat)
       PD_DATA.train(keep = loan_id &train_var SDQ PPY DEL yqtr orig_dte &class_var next_stat);
    set &datasets;
...
* Get the probability of SDQ and PPY;
  proc sort data = PD_DATA.train(keep = yqtr PPY SDQ DEL &macro) out = uni_data;
    by yqtr;
  run;
  ods output Summary = time_uni(keep = _numeric_);
  proc means data = uni_data mean;
    var PPY SDQ DEL &macro;
    by yqtr;
  run;
...
* Get the time series plot;
  %macro time_uni(var, n_var);
    ...
    run;
	proc sgplot data = time_uni;
      series x = yqtr y = DEL_Mean /  legendlabel = "Probability of Delinquent" lineattrs = (color = "cxe34a33" thickness = 2);
      series x = yqtr y = &var._Mean / Y2Axis legendlabel = "&n_var" lineattrs = (color = "cx3182bd" thickness = 2);
      xaxis label = "Year" grid;
      yaxis label = "Probability of Delinquent" grid valuesformat = percent8.2;
      y2axis label = "&n_var";
    run;
    proc sgplot data = time_uni;
  ...
  ods &extension select all;
  %time_uni(QDT_ump, Unemployment Rate: Quarterly Difference);
