...
%let CUR_macro = ump;
%let DEL_macro = gdp_annual_rate hs qdt_ump;


%let CUR_var = OLTV orig_amt loan_age &CUR_macro;
%let DEL_var = CLTV cscore_b Curr_rte dti ratio_upb &DEL_macro;

%macro test(num);
  ods powerpoint exclude all;
* Multinomial Logistic Regression;
  ods output ParameterEstimates = &d_pd._c&num._pa(keep = variable response estimate probchisq);
...
ods powerpoint file = "&p_report/model1.ppt"
               style = Sapphire;
ods html file = "&p_report/model_del.html"
        style = Sapphire;
ods html exclude all;
...
* Get the historical PD for out-of-sample dataset;
  data act_prime act_sub_prime;
    set PD_DATA.test_cur PD_DATA.test_del(keep = orig_dte next_stat yqtr);
/*     where orig_dte < "31DEC2007"d; */
...
/*   ods listing; */
  title "Sub-prime";
  ods output CrossTabFreqs = sub;
/*   ods trace on; */
  proc freq data = act_sub_prime;
    table yqtr*next_stat / nocol nopercent nofreq;
  run;
  title "Prime";
  ods output CrossTabFreqs = prm;
  proc freq data = act_prime;
    table yqtr*next_stat / nocol nopercent nofreq;
  run;
  
/*   proc export data = sub outfile = "&p_pd/sub-prime.csv" dbms = csv; */
/*   run; */
/*   proc export data = prm outfile = "&p_pd/prime.csv" dbms = csv; */
/*   run; */
%mend act_PD;
